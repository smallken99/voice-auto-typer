...
- **雙模式語音輸入**：
  - 🟢 **潤稿模式**：長按 `右 Ctrl` 鍵（≥ 0.5 秒）開始錄音，放開後經由 Whisper 轉錄，再透過 Gemini AI 潤稿後貼上。
...
| 快速鍵 | 模式 | 說明 |
|--------|------|------|
| `右 Ctrl` 長按 ≥ 0.5 秒 | 🟢 潤稿模式 | Whisper 轉錄 → Gemini AI 潤稿 → 自動貼上 |
| `Caps Lock` 長按 ≥ 0.5 秒 | 🟠 快速模式 | Whisper 轉錄 → 直接貼上（不潤稿）|
| 放開按鍵 | — | 停止錄音並開始處理 |

### 操作步驟（潤稿模式）
1. 程式啟動後，螢幕短暫出現「🚀 語音助手已啟動｜右Ctrl: 潤稿｜CapsLock: 快速」提示，右下角系統匣出現綠色小方塊圖示。
2. 將游標點擊到您想要輸入文字的任何地方（例如記事本、Line 聊天框）。
3. **按住 `右 Ctrl` 鍵**（超過 0.5 秒），螢幕顯示「🔴 錄音中...」，此時請開始說話。
4. **放開 `右 Ctrl` 鍵**，螢幕依序顯示「⏳ 語音轉錄中...」→「⏳ AI 潤稿中...」，程式完成後自動貼上。
5. 貼上完成後會顯示「✅ 貼上完成」。
